...
<ParamField body="mode" type="string" default="create">
	Operation mode for the audience : - "create" (default): Create a new
	audience. "append": Add more entries to an existing audience - "replace":
	Replace all entries in an existing audience - "remove": Remove entries from
	an existing audience Required when modifying an existing audience.
</ParamField>
